...
--	How many times was each product added to a cart but not purchased (abandoned)?
--	How many times was each product purchased?
DROP TABLE IF EXISTS CliqueBaitDBUI.dbo.product_conversion_funnel_tb;
...
product_view_and_add_to_cart_times_tb AS (
SELECT
	product_id,
...
product_add_to_cart_but_not_purchase_times_tb AS (
	SELECT
		product_id,
		page_name AS product_name,
...
INTO CliqueBaitDBUI.dbo.product_conversion_funnel_tb
FROM product_view_and_add_to_cart_times_tb AS t1
INNER JOIN product_add_to_cart_but_not_purchase_times_tb AS t2
	ON t1.product_id = t2.product_id
INNER JOIN product_purchase_times_tb AS t3
